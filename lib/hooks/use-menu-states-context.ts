    ...
    if (context) {
      ...
      if (updatedStates.settings) {
        ...
        // Only update TTL if it is set
          ...
          setValue("sttAPIKey", encryptedSttAPIKey, updatedStates.settings.ttl);
          setValue("llmAPIKey", encryptedLlmAPIKey, updatedStates.settings.ttl);
          setValue("ttsAPIKey", encryptedTtsAPIKey, updatedStates.settings.ttl);
        }
      } else {
        // Reset all settings
